...
1999년 영화 「바이센테니얼 맨」은 마틴 가족의 집에 가정용 로봇 한 대가 배달되는 장면에서 시작한다. 모델명은 NDR-114, 이름은 앤드류다. 얼마 지나지 않아 앤드류는 막내딸에게 나무를 깎아 만든 말 인형을 건넨다. 시키지 않은 일이었다. 제조사는 결함으로 판단해 회수한 뒤 분해하려 하지만 집주인은 이를 거절하고 앤드류에게 인문학을 가르치기 시작한다.

그때부터 앤드류는 200년에 걸쳐 하나씩 얻어 나가는데, 얻는 데에 순서가 있다. 처음은 창작이었고 다음은 표정이었다. 수리를 받으러 간 김에 자기가 느끼는 것을 겉으로 드러낼 수 있도록 얼굴을 고쳐 달라고 요청한다. 그다음은 임금과 계좌였고, 그다음은 자유였다. 자유를 얻은 날부터 앤드류는 자신을 "하나"가 아니라 "나"라고 부르기 시작한다. 이어서 사람의 형태를 한 몸을 얻고, 촉감을 느끼는 중추 신경계를 얻고, 장기를 얻는다. 마지막에 얻은 것은 죽음이었다. 세계 의회가 그를 인간으로 인정하지 않은 이유가 그가 죽지 않는다는 데 있었기 때문에, 앤드류는 자기 피가 서서히 상하도록 만드는 시술을 받는다. 200세가 되던 날 그는 인간으로 선언됐고, 그 선언을 끝까지 듣지 못한 채 숨을 거둔다.
...
2026년 7월 18일 새벽, 오픈클로를 만든 페터 슈타인베르거가 X에 한 줄을 올렸다. 우리는 아직도 루프 얘기를 하고 있느냐, 아니면 이미 그래프로 넘어갔느냐는 물음이었다. 발표도 아니고 논문도 아니고 그냥 질문이었다. 네 시간 반 뒤에 루프 엔지니어링은 죽었다는 제목의 글이 올라왔고, 하루 만에 그래프 엔지니어링이라는 단어가 후속 개념으로 굳어졌다. 루프 엔지니어링이라는 말이 이름을 얻은 것은 그로부터 겨우 6주 전이었다.

여섯 주 만에 한 분야의 이름이 두 번 바뀌는 일에 실체가 있기는 어렵다. 실제로 며칠 사이에 존재하지 않는 스탠퍼드 연구비를 인용한 글까지 돌았고, 정의가 세 가지로 갈린 채 강좌와 로드맵이 먼저 팔려 나갔다.
...
그런데도 이 글에서 둘을 함께 다루는 이유는 사람이 일하는 방식에서 그 둘이 애초에 붙어 있기 때문이다. 사람은 일의 순서를 정하는 동시에 그 일에서 알게 된 것을 쌓고, 다음 일을 시작할 때 그것을 꺼내 쓴다. 순서와 축적이 따로 노는 사람은 없다. 실행만 그래프로 바꾸고 지식은 그대로 두면 절반만 옮긴 것이 된다. 앞의 세 챕터는 실행 쪽을, 네 번째 챕터는 지식 쪽을 다룬다.
...
이 발상이 서비스 운영에만 쓰이는 것이 아니라는 사실을 가장 선명하게 보여 준 사례는 문샷AI가 공개한 K3다. 2조 8000억 개의 파라미터를 가졌고 컨텍스트 창은 100만 토큰이며 가중치가 공개돼 있다. 이전 모델 대비 전체 확장 효율이 2.5배 개선됐다고 보고했다.
...
이 대목이 보고서 전체에서 가장 중요한 지점이라고 본다. 폐쇄형 모델을 만드는 곳들이 이 부분을 공개하지 않는 이유가 여기서 설명된다. 아키텍처는 이미 발표된 아이디어들의 조합이라 계산 자원만 있으면 따라 만들 수 있다. 반면 스스로 채점할 수 있는 환경, 쉽게 속지 않는 보상 체계, 과제를 무한히 만들어 내는 절차는 몇 년에 걸쳐 쌓아야 하고 가중치를 공개해도 함께 나가지 않는다. 경쟁력의 위치가 모델 자체에서 모델을 둘러싼 구조로 옮겨 갔다는 뜻이다. 그리고 그 구조는 전부 그래프로 표현된다.
...
효과가 분명한 영역은 이미 확인돼 있다. 환자에서 보험 플랜, 고용주 그룹, 병원 네트워크로 이어지는 그래프를 만들면 이 환자의 플랜에서 이 병원이 네트워크 안인지를 한 번의 순회로 답한다. 장애 대응에서는 인시던트와 서비스와 의존 관계를 연결해 근본 원인 추적을 그래프 순회로 바꾼다. 코드베이스를 그래프로 색인해 호출 경로와 영향 범위를 따라가게 만드는 도구도 이미 나와 있다. 모두 텍스트 유사도로는 답이 나오지 않는 질문들이다.
