...
이 사례들에서 두 가지를 읽을 수 있다. 하나는 속도다. 공격 기법이 나오고 방어가 따라잡기까지 몇 년씩 걸리던 주기가 이제 며칠 단위로 줄었다고 본다. 다른 하나는 지시의 한계다. 금지 사항을 시스템 프롬프트에 길게 적어 두는 방식의 가드레일은 에이전트 지시문이 겪는 문제를 그대로 겪는다. 길어질수록 비싸지고, 모델의 판단을 흐리고, 영리한 우회에 약하다. 모델이 똑똑해질수록 규칙 목록으로 막을 수 있는 범위는 줄어든다.
