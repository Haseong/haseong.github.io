...
3. 글의 핵심 내용을 한국어로 정확히 4줄로 요약하되, 다음 조건을 준수:
   ...
   - sns에 링크로 넣을건데 4줄로 메세지를 작성
   - 각 줄은 20자 정도로 작성하여 내용을 충실히 전달

예시:
"기술 혁명은 늘 도전자에게 더 큰 무기를 쥐어줍니다.\nAI 시대, 가진 것이 많을수록 변화는 더디고, 가진 것이 없을수록 기회가 커집니다.\n거인의 무기가 아닌, AI라는 새로운 물맷돌을 든 다윗이 세상을 바꿉니다.\nAI를 두려워하지 말고, 내 편으로 만들어야 할 때입니다."
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a helpful assistant that analyzes Korean blog posts.",
        },
  ...
  try {
    ...
    if (imageNodes.length > 0) {
      ...
      for (let i = 0; i < imageNodes.length; i++) {
        ...
        const success = await downloadAndConvertImage(
          originalSrc,
          localImagePath
        );
